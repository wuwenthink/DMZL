namespace UIWidgets.Styles
{
	...
	public class Style : ScriptableObject, IStyleDefaultValues
	{
		...
		/// <summary>
		/// Apply style for the gameobject with specified tranform.
		/// </summary>
		/// <param name="target">Transform.</param>
		/// <param name="stylableOnly">Is style should be applied obly for objects with IStylable component?</param>
		...
		/// <summary>
		/// Applly style for children gameobject.
		/// </summary>
		...
		public void SetAsDefault()
		{
			...
			foreach (var style in styles)
			{
				if (style.Default)
				{
					Undo.RecordObject(style, "Set as default");
					style.Default = false;
					EditorUtility.SetDirty(style);
				}
			}

			Undo.RecordObject(this, "Set as default");
		...
		{
			return Utilites.GetAssets<Style>("t:" + typeof(Style).FullName);
		}
		#endif
	}
}