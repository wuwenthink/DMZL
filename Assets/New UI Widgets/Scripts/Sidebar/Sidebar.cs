namespace UIWidgets
{
	...
	public class Sidebar : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IStylable
	{
		/// <summary>
		...
		protected void RectDrag(RectTransform rect, Vector2 closePosition, Vector2 openPosition, Vector2 delta)
		{
			...
			{
				case SidebarAxis.LeftToRight:
					var lx = Mathf.Clamp(rect.anchoredPosition.x + delta.x, closePosition.x, openPosition.x);
					rect.anchoredPosition = new Vector2(lx, rect.anchoredPosition.y);
					break;
				case SidebarAxis.RightToLeft:
					var rx = Mathf.Clamp(rect.anchoredPosition.x + delta.x, openPosition.x, closePosition.x);
					rect.anchoredPosition = new Vector2(rx, rect.anchoredPosition.y);
					break;
				case SidebarAxis.TopToBottom:
					var ty = Mathf.Clamp(rect.anchoredPosition.y + delta.y, openPosition.y, closePosition.y);
					rect.anchoredPosition = new Vector2(rect.anchoredPosition.x, ty);
					break;
				case SidebarAxis.BottomToTop:
					var by = Mathf.Clamp(rect.anchoredPosition.y + delta.y, closePosition.y, openPosition.y);
					rect.anchoredPosition = new Vector2(rect.anchoredPosition.x, by);
					break;
				default:
					throw new NotSupportedException("Unknown sidebar axis: " + Direction);
			}
		...
		protected IEnumerator AnimationCoroutine(RectTransform rect, Vector2 endPos, float rectSpeed)
		{
			...
			do
			{
				animation_position = (Utilites.GetTime(UnscaledTime) - startTime) * acceleration;
		...
		protected IEnumerator ScaleDownAnimationCoroutine()
		{
			...
			do
			{
				animation_position = (Utilites.GetTime(UnscaledTime) - start_time) * acceleration;
