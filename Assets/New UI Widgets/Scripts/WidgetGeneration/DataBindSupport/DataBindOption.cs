...
namespace UIWidgets.DataBindSupport
{
	...
	class DataBindOption
	{
		...
		static public List<DataBindOption> GetOptions(Type type)
		{
		...
		static Dictionary<string, List<string>> GetEvents(Type type, string fieldName)
		{
			...
			foreach (var field in type.GetFields())
			{
				var parametres = GetEventParametres(field, fieldName);
				if (parametres != null)
				{
					result.Add(field.Name, parametres);
				}
			}

			foreach (var property in type.GetProperties())
			{
				var parametres = GetEventParametres(property, fieldName);
				if (parametres != null)
				{
					result.Add(property.Name, parametres);
				}
			}
		...
		static List<string> GetEventParametres(MemberInfo type, string fieldName)
		{
			...
			foreach (var attr in attrs)
			{
				...
				{
					result.Add(Utilites.GetFriendlyTypeName(p.ParameterType));
				}
