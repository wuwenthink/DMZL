		...
		IPointerEnterHandler, IPointerExitHandler
	{
		...
		public LayoutElement LayoutElement
		{
			get
			{
				...
				{
					layoutElement = Utilites.GetOrAddComponent<LayoutElement>(this);
				}
		...
		protected virtual void UpdateCursor()
		{
			if ((regions.NW && ResizeDirections.TopLeft) || (regions.SE && ResizeDirections.BottomRight))
			{
				...
				cursorChanged = true;
				Cursor.SetCursor(CursorNWSETexture, CursorNWSEHotSpot, Compatibility.GetCursorMode());
			}
			else if ((regions.NE && ResizeDirections.TopRight) || (regions.SW && ResizeDirections.BottomLeft))
			{
				globalCursorSetted = true;
				cursorChanged = true;
				Cursor.SetCursor(CursorNESWTexture, CursorNESWHotSpot, Compatibility.GetCursorMode());
			}
			else if ((regions.Top && ResizeDirections.Top) || (regions.Bottom && ResizeDirections.Bottom))
			{
				globalCursorSetted = true;
				cursorChanged = true;
				Cursor.SetCursor(CursorNSTexture, CursorNSHotSpot, Compatibility.GetCursorMode());
			}
			else if ((regions.Left && ResizeDirections.Left) || (regions.Right && ResizeDirections.Right))
			{
				globalCursorSetted = true;
				cursorChanged = true;
				Cursor.SetCursor(CursorEWTexture, CursorEWHotSpot, Compatibility.GetCursorMode());
			}
